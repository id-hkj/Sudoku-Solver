...
def tick(char):
    return re.match('^[0-9]*$', char) is not None and len(char) <= 7 and len(char) >= 3
def help():
    global numm
    numm=StringVar()
    global custom
...
img2 = PhotoImage(file=(path.join(path.dirname(path.abspath(__file__)), "FIRST1.png")))
img3 = PhotoImage(file=(path.join(path.dirname(path.abspath(__file__)), "FIRST1.png")))
img4 = PhotoImage(file=(path.join(path.dirname(path.abspath(__file__)), "FIRST1.png")))
ttk.Button(root, image=img1,command=help).grid(column=6,row=6)
